...
export const NoaChatRoom: React.FC<NoaChatRoomProps> = ({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'msg-1',
      sender: 'noa',
      text: 'שלום! אני נועה AI, מנהלת הסידור, השילוח, צי הרכבים ונרמול ההזמנות של חברת ח. סבן. 🚛✨\n\n🏢 *צי הרכבים והמחסנים שלנו:*\n1. 🏗️ *חכמת (מרצדס מנוף 615-41-002)* — זרוע מנוף להובלות כבדות, בלות חול, סומסום, טיט, מלט ובלוקים מ**מחסן 4 (החרש)**.\n2. 🚚 *עלי (משאית רגילה/פתוחה)* — להובלות ללא מנוף עבור מערכות גבס, פרופילי מתכת, צבעים ודבקים מ**מחסן 1 (התלמיד)**.\n\n💡 שלחו לי כל הזמנה בטקסט חופשי, ואבצע בדיקה אוטומטית של משקל הפריטים, סוג החומר ודרישת מנוף, אנרמל לפריטי מילון לוגיסטי ואשבץ ישירות לסידור העבודה ב-Google Sheets!',
  ...
  const quickPills = [
    '🚚 צי הרכבים והנהגים של ח. סבן',
    '📦 20 שק מלט, 3 בלות חול ו-10 גבס להרצליה',
    '🏗️ מה הלו״ז של חכמת (מנוף 615-41-002)?',
    '🚚 אילו הזמנות משובצות לעלי (מחסן 1)?',
